...
def _good_prod_env(monkeypatch):
    ...
    monkeypatch.setenv("BANK_FEEDS_ENCRYPTION_KEY", "b" * 44)  # Wave 13
...
class TestProductionRequirements:
    ...
    def test_bank_feeds_key_missing_in_prod_fails(self, monkeypatch):
        _good_prod_env(monkeypatch)
        monkeypatch.delenv("BANK_FEEDS_ENCRYPTION_KEY", raising=False)
        result = env_validator.validate_env()
        assert any("BANK_FEEDS_ENCRYPTION_KEY" in e for e in result.errors)

    def test_bank_feeds_key_missing_in_dev_only_warns(self, monkeypatch):
        monkeypatch.setenv("ENVIRONMENT", "development")
        monkeypatch.setenv("JWT_SECRET", "x" * 40)
        monkeypatch.setenv("ADMIN_SECRET", "y" * 20)
        monkeypatch.delenv("BANK_FEEDS_ENCRYPTION_KEY", raising=False)
        result = env_validator.validate_env()
        assert result.is_ok()
        assert any("BANK_FEEDS_ENCRYPTION_KEY" in w for w in result.warnings)
