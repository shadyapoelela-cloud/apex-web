...
def _register_and_login(client):
    ...
    user = {"username": f"cop_{uid}", "email": f"cop_{uid}@test.com",
            "password": "TestPass123!", "display_name": f"Copilot User {uid}"}
...
def _admin_header():
    payload = {
        "sub": "admin-user-id", "username": "admin",
        "roles": ["admin", "registered_user"], "type": "access",
...
class TestCopilot:
    ...
    def test_copilot_create_session(self, client):
        ...
        resp = client.post("/copilot/sessions", headers=h,
    ...
    def test_copilot_chat(self, client):
        ...
        # Chat
        resp = client.post("/copilot/chat", headers=h,
                           json={"message": "ما هي شجرة الحسابات؟", "session_id": sid})
        assert resp.status_code == 200
        data = resp.json()
        assert data.get("success") is True
    ...
    def test_copilot_detect_intent(self, client):
        h = _register_and_login(client)
        resp = client.post("/copilot/detect-intent", headers=h,
                           json={"message": "أريد رفع ميزان مراجعة"})
        assert resp.status_code == 200
...
class TestAccountManagement:
    ...
    def test_update_profile(self, client):
        h = _register_and_login(client)
        # Try PUT, then PATCH
        resp = client.put("/users/me/profile", headers=h,
                          json={"display_name": "Updated Name", "city": "Riyadh"})
        if resp.status_code in (404, 405):
            resp = client.patch("/users/me/profile", headers=h,
                                json={"display_name": "Updated Name", "city": "Riyadh"})
        assert resp.status_code in (200, 400, 404, 405, 422)
    ...
    def test_change_password_wrong_current(self, client):
        h = _register_and_login(client)
        resp = client.put("/users/me/security/password", headers=h,
    ...
    def test_request_account_closure(self, client):
        h = _register_and_login(client)
        resp = client.post("/account/closure", headers=h,
                           json={"type": "temporary", "reason": "testing"})
...
class TestLegalDocuments:
    ...
    def test_accept_legal(self, client):
        h = _register_and_login(client)
        resp = client.post("/legal/accept", headers=h,
...
class TestKnowledgeFeedback:
    def test_submit_feedback(self, client):
        h = _register_and_login(client)
        resp = client.post("/coa/knowledge-feedback", headers=h,
                           json={"client_id": "test-client", "feedback_text": "test feedback",
...
class TestKnowledgeBrain:
    ...
    def test_create_concept(self, client, auth_header):
        ...
        resp = client.post("/knowledge/concepts", headers=auth_header,
                           json={"canonical_name": f"test_concept_{uid}",
                                 "name_ar": f"مفهوم {uid}", "name_en": f"Concept {uid}",
                                 "category": "asset"})
        assert resp.status_code in (200, 201)

    def test_resolve_concept(self, client, auth_header):
        resp = client.post("/knowledge/resolve", headers=auth_header,
                           json={"text": "الأصول المتداولة"})
