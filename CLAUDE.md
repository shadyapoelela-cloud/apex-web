...
- **Pilot route security pattern (G-PILOT-REPORTS-TENANT-AUDIT, 2026-05-08).** Pilot models (`Entity`, `JournalEntry`, `GLPosting`, `GLAccount`, `FiscalPeriod`, …) do **not** inherit from `TenantMixin`, so the application-layer `attach_tenant_guard` does **not** cover them — every pilot route taking `entity_id` (path or payload) MUST call `assert_entity_in_tenant(db, entity_id, current_user)` from `app.pilot.security.tenant_guards`. Cross-tenant probes return **404 with the generic `"Entity not found"` body** — same shape as a genuinely missing id, so the status code leaks no existence signal (anti-enumeration). The helper emits a structured `TENANT_GUARD_VIOLATION` warning log on rejection so SOC dashboards can still detect probing. Pattern + history in `docs/PILOT_TENANT_GUARD_PATTERN.md`. History: G-TB-REAL-DATA-AUDIT (PR #174) discovered the gap on the trial-balance endpoint; G-PILOT-REPORTS-TENANT-AUDIT closed it across all 32 vulnerable pilot routes (gl reports + entity CRUD + ZATCA/GOSI/WPS/UAE-CT/VAT + purchasing + AI extraction + sales-invoice listing). Open follow-up `G-PILOT-CATALOG-TENANT-AUDIT` for the `catalog_routes.py` `/tenants/{tenant_id}/...` shape; `G-PILOT-PO-PI-TENANT-AUDIT` for routes resolving by PO/PI/payment id directly.
