...
- **Pilot route security pattern (G-PILOT-TENANT-AUDIT-FINAL, 2026-05-08).** Pilot models do **not** inherit from `TenantMixin`, so the application-layer `attach_tenant_guard` does **not** cover them. Every pilot route MUST call one of three helpers from `app.pilot.security.tenant_guards` based on its shape: (1) `assert_entity_in_tenant(db, entity_id, current_user)` for `entity_id` in path/payload; (2) `assert_tenant_matches_user(tenant_id, current_user)` for `/tenants/{tid}/...` URLs; (3) `assert_resource_in_tenant(db, Model, resource_id, current_user)` for resources resolved directly by id (PO, PI, JE, Customer, Vendor, Product, Branch, …) — defaults to reading the `tenant_id` column off the loaded row, accepts a `tenant_resolver` callback for models without one (e.g., `ProductAttributeValue`). All three return **404 with the generic `"Entity not found"` / `"Resource not found"` body** on rejection — same shape as a genuinely missing id, so the status code leaks no existence signal (anti-enumeration). All three emit structured `TENANT_GUARD_VIOLATION` warning logs for SOC dashboards. Pattern + decision table in `docs/PILOT_TENANT_GUARD_PATTERN.md`; final summary across all three audits in `docs/PILOT_TENANT_AUDIT_FINAL_2026-05-08.md`. History: G-TB-REAL-DATA-AUDIT (PR #174, 1 route) → G-PILOT-REPORTS-TENANT-AUDIT (PR #175, 32 routes) → G-PILOT-TENANT-AUDIT-FINAL (this PR, 85 routes). **Issue #3 (Multi-Tenancy Isolation) FULLY CLOSED across 117 pilot routes.**
- **Demo data (G-DEMO-DATA-SEEDER, 2026-05-08):** users can populate their own tenant via `POST /api/v1/account/seed-demo-data` (JWT-gated, reads `tenant_id` from the claim). Ops can use `POST /admin/seed-demo-data?tenant_id=…` (X-Admin-Secret) for customer onboarding. v1 seeds master data only — 5 customers + 5 vendors + 15 products. Idempotent. `force=true` appends with code-suffix collision avoidance. JE / invoice / payment seeding is deferred to G-DEMO-DATA-SEEDER-V2 (separate PR). See `docs/DEMO_DATA_GUIDE_2026-05-08.md`.
- **Sales invoice flow (G-SALES-INVOICE-UX-COMPLETE, 2026-05-10):** the canonical user path is **list → details → action**. List row tap routes to `/app/erp/finance/sales-invoices/:invoiceId` (`SalesInvoiceDetailsScreen`) — **NOT** to the JE-builder. The JE is reachable only via the explicit "عرض القيد" button inside the details screen. Pre-this-PR `_openInvoice` jumped straight to `/je-builder/{jeId}`, skipping the invoice payload entirely. The new screen also renders a Phase 1 ZATCA QR (TLV → base64 via `apex_finance/lib/core/zatca_tlv.dart`) and houses the `+ تسجيل دفع` action that opens `CustomerPaymentModal`. Payment posts auto-build a 2-leg JE (DR Cash 1110 / Bank 1120 / Cheques-on-hand 1310 — by method; CR AR 1130) — see `_post_customer_payment_je` in `customer_routes.py`. Overpayments are rejected with 409. New seeded CoA account: **1310 Cheques on Hand** (subcategory `cash_equivalent`). See `docs/SALES_INVOICE_UX_COMPLETE_2026-05-10.md`.
