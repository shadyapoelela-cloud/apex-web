...
- **Pilot models bypass `attach_tenant_guard` (G-TB-REAL-DATA-AUDIT, 2026-05-08).** Pilot tables (`pilot_entities`, `pilot_journal_entries`, `pilot_gl_postings`, `pilot_gl_accounts`, `pilot_fiscal_periods`, …) declare `tenant_id` as a plain column instead of inheriting `TenantMixin`, so the global guard does **not** filter their queries automatically. Routes that resolve a pilot entity by id MUST pass `current_user` to the entity-resolution helper and assert `entity.tenant_id == current_user["tenant_id"]` explicitly — the pattern in `app/pilot/routes/gl_routes.py::_entity_or_404`. Cross-tenant probes return **403** (not 404 — 404 is reserved for genuinely missing ids so status codes leak no existence signal). Routes still missing this check (income-statement, balance-sheet, cash-flow, comparative) are tracked as `G-PILOT-REPORTS-TENANT-AUDIT`.
