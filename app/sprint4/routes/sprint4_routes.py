    ...
    try:
        ...
        if dup:
            ...
            _exec(
                ...
                {"id": conflict_id, "text": body.alias_text, "c1": concept_id, "c2": None, "now": _now()},
            )
...
def list_pending_aliases(page: int = 1, page_size: int = 20):
    ...
    try:
        total = _exec(
            db, "SELECT COUNT(*) FROM knowledge_concept_aliases WHERE review_status = 'pending_review'"
        ).fetchone()[0]
...
def resolve_term(body: ResolveTermRequest):
    ...
    try:
        ...
        concepts = [
            dict(zip(["id", "canonical_name_ar", "canonical_name_en", "domain_pack", "authority_level"], r))
            for r in _exec(
                db,
        ...
        if result["match_type"] == "no_match":
            _exec(
                ...
                {"id": str(uuid.uuid4()), "text": raw_term, "lang": language, "sys": source_system, "now": _now()},
            )
            db.commit()
...
def list_candidate_rules(domain_pack: Optional[str] = None, page: int = 1, page_size: int = 20):
    ...
    try:
        ...
        return {
            "success": True,
            "data": {"total": total, "page": page, "candidates": [dict(zip(keys, r)) for r in rows]},
        }
    except Exception:
        logging.error("Knowledge Brain operation failed", exc_info=True)
        raise HTTPException(500, "Internal server error")
    ...
    try:
        ...
        for r in rows:
            try:
                ...
                logic = {}
            rules.append(
                {"id": r[0], "rule_name": r[1], "domain_pack": r[2], "rule_logic_json": logic, "authority_level": r[4]}
            )
        ...
        for c in conflicts:
            ...
            try:
                _exec(
                    ...
                    {
                        "id": cid,
                        "text": f"rule_conflict:{c['rule_1_id'][:8]}_vs_{c['rule_2_id'][:8]}",
                        "c1": c["rule_1_id"],
                        "c2": c["rule_2_id"],
                        "type": "rule_conflict",
...
def list_conflicts(status: str = "pending", page: int = 1, page_size: int = 20):
    ...
    try:
        total = _exec(
            db, "SELECT COUNT(*) FROM knowledge_alias_conflicts WHERE conflict_status = :s", {"s": status}
        ).fetchone()[0]
...
def brain_status():
    ...
    db = _db()
    try:
        concepts_total = _exec(db, "SELECT COUNT(*) FROM knowledge_concepts WHERE validity_status='active'").fetchone()[
            0
        ]
        aliases_approved = _exec(db, "SELECT COUNT(*) FROM knowledge_concept_aliases WHERE is_approved=1").fetchone()[0]
        aliases_pending = _exec(
            db, "SELECT COUNT(*) FROM knowledge_concept_aliases WHERE review_status='pending_review'"
        ).fetchone()[0]
        rules_active = _exec(
            db, "SELECT COUNT(*) FROM active_knowledge_rules WHERE validity_status='active'"
        ).fetchone()[0]
        rules_pending = _exec(
            db, "SELECT COUNT(*) FROM knowledge_candidate_rules WHERE submission_status='pending_review'"
        ).fetchone()[0]
        conflicts_open = _exec(
            db, "SELECT COUNT(*) FROM knowledge_alias_conflicts WHERE conflict_status='pending'"
        ).fetchone()[0]
        source_systems = _exec(db, "SELECT COUNT(*) FROM source_system_profiles").fetchone()[0]

        readiness = (
            "operational"
            if concepts_total >= 10 and aliases_approved >= 20
            else "partial" if concepts_total >= 1 else "empty"
        )
