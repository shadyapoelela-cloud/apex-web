...
def debug_register_trace():
    """Diagnostic endpoint — runs a register manually, bypassing the
    register() service's try/except to surface the actual root cause."""
    import traceback
    import random
    ...
    from app.phase1.models.platform_models import (
        SessionLocal, gen_uuid, User, UserProfile, Role, UserRole,
        Plan, UserSubscription, PlanFeature, SubscriptionEntitlement,
        UserSecurityEvent, RoleCode, PlanCode,
    )
    ...
    try:
        ...
        try:
            user = User(
                ...
                password_hash=hash_password("ApexDbg@2026"),
                status="active",
            )
            db.add(user)
            db.flush()
            report["steps"].append("user: ok")
        except Exception as e:
            report["failure_at"] = "User"
            report["exception_type"] = type(e).__name__
            report["exception_message"] = str(e)
        ...
        try:
            ...
            db.flush()
            report["steps"].append("user_profile: ok")
        except Exception as e:
            report["failure_at"] = "UserProfile"
            report["exception_type"] = type(e).__name__
            report["exception_message"] = str(e)
            report["traceback"] = traceback.format_exc()
            db.rollback()
        ...
        try:
            ...
            if role:
                ...
                db.flush()
                report["steps"].append("user_role: ok")
            else:
                report["steps"].append("user_role: role not found")
        except Exception as e:
            report["failure_at"] = "UserRole"
            report["exception_type"] = type(e).__name__
            report["exception_message"] = str(e)
            report["traceback"] = traceback.format_exc()
            db.rollback()
        ...
        # Step 4: UserSubscription + Entitlements
        try:
            ...
            if free_plan:
                ...
                db.add(sub)
                db.flush()
                report["steps"].append("user_subscription: ok")
                ...
                db.flush()
                report["steps"].append(f"entitlements: {len(features)}")
        except Exception as e:
            report["failure_at"] = "UserSubscription/Entitlements"
            report["exception_type"] = type(e).__name__
            report["exception_message"] = str(e)
            report["traceback"] = traceback.format_exc()
            db.rollback()
        ...
        # Step 5: UserSecurityEvent
        try:
            db.add(UserSecurityEvent(
                id=gen_uuid(), user_id=user.id,
                event_type="registration", ip_address="",
                details={"method": "email"},
            ))
            db.flush()
            report["steps"].append("user_security_event: ok")
        except Exception as e:
            report["failure_at"] = "UserSecurityEvent"
            report["exception_type"] = type(e).__name__
            report["exception_message"] = str(e)
            report["traceback"] = traceback.format_exc()
            db.rollback()
        ...
        try:
            db.commit()
            report["steps"].append("commit: ok")
        except Exception as e:
            report["failure_at"] = "commit"
            report["exception_type"] = type(e).__name__
            report["exception_message"] = str(e)
            report["traceback"] = traceback.format_exc()
            db.rollback()
        ...
        report["result"] = "SUCCESS — user created"
        return report
    finally:
        db.close()
