            ...
            "data": [
                {
                    ...
                    "category": s.category,
                    "icon": s.icon,
                    "requires_coa": s.requires_coa,
                    "requires_tb": s.requires_tb,
                    "min_plan": s.min_plan,
    ...
    try:
        ...
        return {
            ...
            "data": {
                ...
                "requires_coa": svc.requires_coa,
                "requires_tb": svc.requires_tb,
                "stages": [
                    {
...
async def create_service_case(req: CreateServiceCaseRequest, user: dict = Depends(get_current_user)):
    ...
    try:
        svc = (
            ...
            .filter(ServiceCatalog.service_code == req.service_code, ServiceCatalog.is_active == True)
            .first()
        )
        if not svc:
            raise HTTPException(status_code=404, detail="Service not found or inactive")
        first_stage = (
            db.query(ServiceWorkflowStage)
            .filter(ServiceWorkflowStage.service_id == svc.id)
        ...
        case = ServiceCase(
            ...
            current_stage=first_stage.stage_code if first_stage else None,
            notes=req.notes,
            created_by=user["sub"],
            ...
            "data": [
                {
                    ...
                    "local_std_ref": t.local_std_ref,
                    "international_ref": t.international_ref,
                }
                for t in items
            ],
        }
...
async def create_sample(case_id: str, req: CreateSampleRequest, user: dict = Depends(get_current_user)):
    ...
    try:
        sample = AuditSample(
            ...
            population_ref=req.population_ref,
    ...
    try:
        ...
        return {
            ...
            "data": [
                {
                    "id": w.id,
                    "procedure_code": w.procedure_code,
                    "area": w.area,
                    "result": w.result,
                    "severity": w.severity,
...
async def create_finding(case_id: str, req: CreateFindingRequest, user: dict = Depends(get_current_user)):
    ...
    try:
        finding = AuditFinding(
            service_case_id=case_id,
            finding_code=f"F-{gen_uuid()[:8].upper()}",
            area=req.area,
            title_ar=req.title_ar,
...
async def list_findings(case_id: str):
    ...
    try:
        items = (
            ...
            .filter(AuditFinding.service_case_id == case_id)
            ...
            "data": [
                {
                    ...
                    "severity": f.severity,
                    "status": f.status,
                    "impact_on_report": f.impact_on_report,
                }
                for f in items
            ],
        }
