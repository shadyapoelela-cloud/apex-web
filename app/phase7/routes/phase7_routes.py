...
async def list_task_types():
    ...
    try:
        ...
        all_reqs = (
            db.query(TaskDocumentRequirement)
            .filter(TaskDocumentRequirement.task_type_id.in_(type_ids))
            .order_by(TaskDocumentRequirement.sort_order)
            .all()
        )
        ...
        for tt in types:
            ...
            inputs = [
                {
                    "id": r.id,
                    "name_ar": r.document_name_ar,
                    "name_en": r.document_name_en,
                    "is_mandatory": r.is_mandatory,
                }
                for r in reqs
                if r.requirement_type == DocRequirementType.input_required
            ]
            outputs = [
                {
                    "id": r.id,
                ...
                for r in reqs
                if r.requirement_type == DocRequirementType.output_required
            ]
...
async def get_task_type(code: str):
    ...
    try:
        ...
        reqs = (
            db.query(TaskDocumentRequirement)
            .filter(TaskDocumentRequirement.task_type_id == tt.id)
            .order_by(TaskDocumentRequirement.sort_order)
            .all()
        )
        ...
        return {
            ...
            "data": {
                ...
                "name_en": tt.name_en,
                "input_requirements": [
                    {"id": r.id, "name_ar": r.document_name_ar, "is_mandatory": r.is_mandatory}
                    for r in reqs
                    if r.requirement_type == DocRequirementType.input_required
                ],
                "output_requirements": [
...
async def get_provider_compliance(provider_id: str, limit: int = 50, offset: int = 0, user=Depends(get_current_user)):
    ...
    try:
        ...
        flags = (
            db.query(ProviderComplianceFlag)
            .filter(ProviderComplianceFlag.provider_id == provider_id)
            .order_by(ProviderComplianceFlag.created_at.desc())
            .limit(min(limit, 100))
            .offset(offset)
            .all()
        )
        ...
        active_suspension = (
            db.query(ProviderSuspension)
            .filter(ProviderSuspension.provider_id == provider_id, ProviderSuspension.status == SuspensionStatus.active)
            .first()
        )
        ...
        return {
            ...
            "data": {
                ...
                "flags": [
                    {
                        "id": f.id,
                        "action": f.action.value if f.action else str(f.action),
...
async def get_result_details(analysis_id: str, user=Depends(get_current_user)):
    ...
    try:
        ...
        explanations = db.query(P7ResultExplanation).filter(P7ResultExplanation.analysis_id == analysis_id).all()
...
async def generate_result_explanation(analysis_id: str, user=Depends(get_current_user)):
    ...
    try:
        ...
        for key, summary_ar, summary_en in result_keys:
            existing = (
                db.query(P7ResultExplanation)
                .filter(P7ResultExplanation.analysis_id == analysis_id, P7ResultExplanation.result_key == key)
                .first()
            )
            if not existing:
                db.add(
...
async def resolve_entitlements(feature: str, user=Depends(get_current_user)):
    ...
    try:
        ...
        sub = (
            ...
            .filter(UserSubscription.user_id == u.id, UserSubscription.is_active == True)
            .first()
        )
        ...
        return {
            ...
            "data": {
                ...
                "feature": feature,
                "access": access,
                "allowed": access not in [False, 0, "view"],
            },
        }
    finally:
        db.close()
...
async def list_audit_events(
    user_id: Optional[str] = None, action: Optional[str] = None, limit: int = 50, user=Depends(get_current_user)
):
