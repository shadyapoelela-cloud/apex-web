...
class _SAS extends State<SlideAuthScreen> {
  ...
  void _sl(int p) {
    _pc.animateToPage(p, duration: const Duration(milliseconds: 350), curve: Curves.easeInOut);
  }

  Future<void> _login() async {
    setState(() { _ll = true; _le = null; });
    try {
      final url = 'https://apex-api-ootk.onrender.com/auth/login';
      final body = jsonEncode({
        'username_or_email': _lu.text.trim(),
        'password': _lp.text,
      });
  ...
  Future<void> _register() async {
    ...
    try {
      ...
      final body = jsonEncode({
        ...
        'password': _rp.text,
        'display_name': _rn.text.trim(),
        'phone': _cc + _rph.text.trim(),
      });
      final r = await http.post(Uri.parse(url),
        headers: {'Content-Type': 'application/json'}, body: body);
      final d = jsonDecode(r.body);
  ...
  Widget _tb(String l, int i) => Expanded(child: GestureDetector(
    onTap: () { setState(() => _pg = i); _sl(i); },
    child: AnimatedContainer(
      duration: const Duration(milliseconds: 250),
      padding: const EdgeInsets.symmetric(vertical: 10),
      decoration: BoxDecoration(
        color: _pg == i ? AC.gold : Colors.transparent,
        borderRadius: BorderRadius.circular(10),
      ),
      child: Center(child: Text(l, style: TextStyle(
        color: _pg == i ? AC.navy : AC.ts, fontSize: 13, fontWeight: FontWeight.bold))),
    ),
  ));
  ...
  Widget _tf(TextEditingController c, String l, IconData ic, {bool ltr = false}) => TextField(
    controller: c, style: const TextStyle(color: AC.tp),
    textDirection: ltr ? TextDirection.ltr : null,
    decoration: InputDecoration(
      labelText: l, prefixIcon: Icon(ic, color: AC.gold, size: 20),
  ...
  Widget _pf(TextEditingController c, String l, bool o, VoidCallback t, {VoidCallback? sub}) => TextField(
    ...
    decoration: InputDecoration(
      labelText: l, prefixIcon: const Icon(Icons.lock_outlined, color: AC.gold, size: 20),
  ...
  Widget _btn(String l, bool ld, VoidCallback fn) => SizedBox(width: double.infinity, height: 46,
    child: ElevatedButton(onPressed: ld ? null : fn,
      ...
      child: ld
        ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, color: AC.navy))
        : Text(l, style: const TextStyle(color: AC.navy, fontSize: 15, fontWeight: FontWeight.bold)),
    ),
  );

  Widget _errW(String m) => Container(
  ...
  @override
  void dispose() {
    _pc.dispose(); _lu.dispose(); _lp.dispose();
    _rn.dispose(); _ru.dispose(); _rem.dispose();
    _rp.dispose(); _rp2.dispose(); _rph.dispose();
    super.dispose();
  }
}