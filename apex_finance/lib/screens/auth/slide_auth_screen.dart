...
class _SlideAuthState extends State<SlideAuthScreen> {
  ...
  Future<void> _register() async {
    ...
    try {
      final r = await http.post(Uri.parse('\$_api/auth/register'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({'username': _ru.text.trim(), 'email': _re2.text.trim(), 'password': _rp.text, 'display_name': _rn.text.trim()}));
      final d = jsonDecode(r.body);
    ...
    return Scaffold(
      ...
      body: Center(child: Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          // Logo
          const Icon(Icons.account_balance, color: AC.gold, size: 42),
          const SizedBox(height: 6),
          const Text('APEX', style: TextStyle(color: AC.gold, fontSize: 28, fontWeight: FontWeight.w900, letterSpacing: 4)),
          Text('منصة التحليل المالي', style: TextStyle(color: AC.ts, fontSize: 10)),
          ...
          const SizedBox(height: 20),
          // Slides
          SizedBox(height: 380, child: PageView(controller: _pc, onPageChanged: (i) => setState(() => _pg = i), children: [_loginSlide(), _registerSlide()])),
        ]),
      )),
    );
  }
  ...
  Widget _loginSlide() => SingleChildScrollView(child: Column(children: [
    ...
    const SizedBox(height: 16), _or(), const SizedBox(height: 12), _soc(),
  ]));
  ...
  Widget _bt(String l, bool ld, VoidCallback fn) => SizedBox(width: double.infinity, height: 48, child: ElevatedButton(onPressed: ld ? null : fn,
  ...
  Widget _err(String m) => Container(width: double.infinity, margin: const EdgeInsets.only(bottom: 12), padding: const EdgeInsets.all(10),
  ...
  Widget _soc() => Row(children: [
    Expanded(child: OutlinedButton.icon(onPressed: () {}, style: OutlinedButton.styleFrom(side: const BorderSide(color: AC.bdr), padding: const EdgeInsets.symmetric(vertical: 11), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
  ...
  @override
  void dispose() { _pc.dispose(); _lu.dispose(); _lp.dispose(); _rn.dispose(); _ru.dispose(); _re2.dispose(); _rp.dispose(); _rp2.dispose(); super.dispose(); }
}