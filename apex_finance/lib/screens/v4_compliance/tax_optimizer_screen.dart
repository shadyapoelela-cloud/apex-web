...
class _TaxOptimizerScreenState extends State<TaxOptimizerScreen> with SingleTickerProviderStateMixin {
  ...
  Widget _hero() => Container(padding: const EdgeInsets.all(20),
    ...
    child: Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        SizedBox(height: 4),
        Text('محاكاة سيناريوهات + اكتشاف فرص التوفير الضريبي', style: TextStyle(color: core_theme.AC.ts, fontSize: 13)),
      ])),
    ]),
  );
  ...
  Widget _kpis() {
    ...
    return Container(padding: const EdgeInsets.all(12), color: Colors.white, child: Row(children: [
      ...
      Expanded(child: _kpi('بعد التحسين', '11.8%', Icons.trending_down, core_theme.AC.gold)),
    ...
    return Card(margin: const EdgeInsets.only(bottom: 10), child: Padding(padding: const EdgeInsets.all(14),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          ...
          Icon(Icons.arrow_forward, color: core_theme.AC.ts),
          Expanded(child: _compare('بالسيناريو', '${(s.projectedTax/1000).toStringAsFixed(0)}K', const Color(0xFF2E7D32))),
          const SizedBox(width: 8),
          Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: core_theme.AC.gold.withValues(alpha: 0.2), borderRadius: BorderRadius.circular(6)),
            child: Text('توفير\n${((s.currentTax - s.projectedTax)/1000).toStringAsFixed(0)}K',
              style: TextStyle(color: core_theme.AC.gold, fontSize: 11, fontWeight: FontWeight.bold), textAlign: TextAlign.center)),
        ]),
      ]),
    ));
  });
    ...
    return Card(margin: const EdgeInsets.only(bottom: 8), child: Padding(padding: const EdgeInsets.all(14),
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(o.action, style: TextStyle(fontSize: 11, color: core_theme.AC.tp)),
          Text('المرجع: ${o.reference}', style: TextStyle(fontSize: 10, color: core_theme.AC.ts)),
        ])),
        Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          ...
          Text('توفير سنوي', style: TextStyle(fontSize: 9, color: core_theme.AC.ts)),
        ]),
      ]),
    ));
  });
  ...
  Widget _treatiesTab() => ListView.builder(padding: const EdgeInsets.all(12), itemCount: _treaties.length, itemBuilder: (_, i) {
    ...
    return Card(margin: const EdgeInsets.only(bottom: 8), child: ListTile(
      ...
      trailing: Text('منذ ${t.year}', style: TextStyle(fontSize: 11, color: core_theme.AC.ts)),
    ));
  });
  ...
  Widget _analyticsTab() => ListView(padding: const EdgeInsets.all(14), children: [
    ...
    _insight('📊 المعدل الفعلي الحالي', '15.2% — يمكن خفضه إلى 11.8% بالتحسينات', core_theme.AC.gold),
