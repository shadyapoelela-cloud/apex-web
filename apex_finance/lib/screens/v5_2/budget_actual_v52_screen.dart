...
class _BudgetActualV52ScreenState extends State<BudgetActualV52Screen> {
  ...
  Widget build(BuildContext context) {
    return MultiViewTemplate(
      ...
      filterChips: [
        FilterChipDef(id: 'revenue', labelAr: 'الإيرادات', color: Colors.green, count: _lines.where((l) => l.category == 'إيراد').length, active: _filter == 'revenue'),
        FilterChipDef(id: 'cost', labelAr: 'التكاليف', color: Colors.orange, count: _lines.where((l) => l.category == 'تكلفة').length, active: _filter == 'cost'),
        FilterChipDef(id: 'opex', labelAr: 'المصروفات', color: Colors.red, count: _lines.where((l) => l.category == 'تشغيلية').length, active: _filter == 'opex'),
      ],
            ...
            children: _lines.map((l) {
              ...
              return Container(
                ...
                child: Row(children: [
                  ...
                  SizedBox(width: 110, child: Text((ytdActual / 1000).toStringAsFixed(0) + 'K', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: isFavorable ? Colors.green : variance == 0 ? Colors.black87 : Colors.red, fontFamily: 'monospace'), textAlign: TextAlign.end)),
                  SizedBox(width: 90, child: Text('${variance >= 0 ? '+' : ''}${variancePct.toStringAsFixed(1)}%', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: isFavorable ? Colors.green : Colors.red), textAlign: TextAlign.end)),
                ]),
              );
            }).toList(),
          )),
  ...
  Widget _cell(double budget, double actual, bool isRevenue) {
    if (actual == 0) {
      return Container(padding: const EdgeInsets.symmetric(horizontal: 4), child: Text((budget.abs() / 1000).toStringAsFixed(0) + 'K', style: TextStyle(fontSize: 10, fontFamily: 'monospace', color: Colors.grey.shade500), textAlign: TextAlign.center));
    }
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: isFavorable ? Colors.green.withOpacity(0.05) : Colors.red.withOpacity(0.05), borderRadius: BorderRadius.circular(3)),
      child: Column(children: [
        Text((actual.abs() / 1000).toStringAsFixed(0) + 'K', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: isFavorable ? Colors.green : Colors.red), textAlign: TextAlign.center),
        Text('${variance >= 0 ? '+' : ''}${(variance.abs() / 1000).toStringAsFixed(0)}K', style: TextStyle(fontSize: 8, color: isFavorable ? Colors.green.shade700 : Colors.red.shade700), textAlign: TextAlign.center),
      ]),
    );
  }
  ...
  Widget _totalRow() {
    ...
    return Container(
      ...
      child: Row(children: [
        const SizedBox(width: 180, child: Text('الإجماليات', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: _navy))),
        ...
        SizedBox(width: 110, child: Text('${(actualTotal / 1e6).toStringAsFixed(2)}M', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: _gold, fontFamily: 'monospace'), textAlign: TextAlign.end)),
        SizedBox(width: 90, child: Text('${(((actualTotal - budgetTotal) / budgetTotal).abs() * 100).toStringAsFixed(1)}%', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: Colors.green), textAlign: TextAlign.end)),
      ]),
    );
  }

  Color _catColor(String cat) {
    if (cat == 'إيراد') return Colors.green;
    if (cat == 'تكلفة') return Colors.orange;
    return Colors.red;
  }
  ...
  Widget _chart() {
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('أعلى 6 انحرافات YTD (بالقيمة)', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: _navy)),
        ...
        ...variances.map((v) {
          ...
          return Padding(padding: const EdgeInsets.only(bottom: 14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              Expanded(child: ClipRRect(borderRadius: BorderRadius.circular(3), child: LinearProgressIndicator(value: pct, minHeight: 22, backgroundColor: Colors.grey.shade100, color: isFav ? Colors.green : Colors.red))),
              const SizedBox(width: 10),
              SizedBox(width: 140, child: Text('${v.$5 >= 0 ? '+' : ''}${(v.$5.abs() / 1000).toStringAsFixed(0)}K ر.س', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: isFav ? Colors.green : Colors.red), textAlign: TextAlign.end)),
            ]),
          ]));
        }),
      ]),
