...
class _VatReturnV52ScreenState extends State<VatReturnV52Screen> {
  ...
  Widget _step1() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Text('اختر الفترة الضريبية:', style: TextStyle(fontSize: 13, color: Colors.black54)),
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Icon(Icons.event, color: selected ? _gold : Colors.black54, size: 20),
            const Spacer(),
            if (current) Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: Colors.orange.withOpacity(0.12), borderRadius: BorderRadius.circular(4)), child: const Text('الحالي', style: TextStyle(fontSize: 9, fontWeight: FontWeight.w800, color: Colors.orange))),
          ]),
          const SizedBox(height: 8),
          Text(period, style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: selected ? _navy : Colors.black87)),
          Text(dates, style: const TextStyle(fontSize: 11, color: Colors.black54)),
          const SizedBox(height: 6),
          Row(children: [
            const Icon(Icons.schedule, size: 11, color: Colors.black54),
            const SizedBox(width: 4),
            Text('استحقاق: $due', style: const TextStyle(fontSize: 10, color: Colors.black54)),
          ]),
        ]),
      ),
    );
  ...
  Widget _step2() {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(
        ...
        decoration: BoxDecoration(color: Colors.green.withOpacity(0.06), borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.green.withOpacity(0.3))),
      ...
      const SizedBox(height: 16),
      const Text('المصادر المُدرجة في الإقرار:', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700)),
      const SizedBox(height: 8),
      ...sources.map((s) => Container(
            ...
            child: Row(children: [
              Icon(s.$4 ? Icons.check_circle : Icons.radio_button_unchecked, color: s.$4 ? Colors.green : Colors.grey),
              const SizedBox(width: 10),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(s.$1, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700)),
                Text('${s.$2} · ${s.$3}', style: const TextStyle(fontSize: 11, color: Colors.black54)),
              ])),
            ]),
          )),
      ...
      CheckboxListTile(
        ...
        title: const Text('أؤكّد صحة المصادر أعلاه', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700)),
        subtitle: const Text('سيتم استخدامها لحساب VAT Output / Input', style: TextStyle(fontSize: 10)),
        activeColor: _gold,
        dense: true,
      ),
    ]);
  ...
  Widget _step3() {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(
        ...
        child: Row(children: [
          const Icon(Icons.arrow_circle_up, color: _gold, size: 28),
          const SizedBox(width: 12),
          const Expanded(child: Text('ضريبة المخرجات (Output VAT)', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: _navy))),
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            Text('${totalVAT.toStringAsFixed(0)} ر.س', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w800, color: _gold)),
            Text('من مبيعات ${(totalSales / 1e6).toStringAsFixed(2)} مليون', style: const TextStyle(fontSize: 10, color: Colors.black54)),
          ]),
        ]),
      ),
      const SizedBox(height: 16),
  ...
  Widget _step4() {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(
        padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(color: Colors.green.withOpacity(0.06), borderRadius: BorderRadius.circular(10), border: Border.all(color: Colors.green)),
        child: Row(children: [
          const Icon(Icons.arrow_circle_down, color: Colors.green, size: 28),
          const SizedBox(width: 12),
          const Expanded(child: Text('ضريبة المدخلات (Input VAT)', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: _navy))),
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            Text('${totalVAT.toStringAsFixed(0)} ر.س', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w800, color: Colors.green)),
            Text('من مشتريات ${(totalPurch / 1e6).toStringAsFixed(2)} مليون', style: const TextStyle(fontSize: 10, color: Colors.black54)),
          ]),
        ]),
      ),
      const SizedBox(height: 16),
  ...
  Widget _step5() {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Text('التسويات المقترحة لنهاية الفترة:', style: TextStyle(fontSize: 13, color: Colors.black54)),
      const SizedBox(height: 12),
      ...adjustments.map((a) => Container(
            ...
            decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.grey.shade200)),
            child: Row(children: [
              Icon(a.$3 ? Icons.check_circle : Icons.radio_button_unchecked, color: a.$3 ? Colors.green : Colors.grey),
              const SizedBox(width: 10),
              Expanded(child: Text(a.$1, style: const TextStyle(fontSize: 13))),
              Text('${a.$2 >= 0 ? '+' : ''}${a.$2.toStringAsFixed(0)} ر.س', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: a.$2 >= 0 ? Colors.green : Colors.red)),
            ]),
          )),
      const SizedBox(height: 12),
      CheckboxListTile(
        ...
        title: const Text('راجعت التسويات وقمت بتطبيق ما يلزم', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700)),
        activeColor: _gold,
        dense: true,
      ),
    ]);
    ...
    return SingleChildScrollView(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('ملخّص الإقرار الضريبي', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: _navy)),
            const SizedBox(height: 4),
            Text('الفترة: $_period · ZATCA VAT Return', style: const TextStyle(fontSize: 11, color: Colors.black54)),
            const Divider(height: 24),
            _kv('ضريبة المخرجات Output VAT', '+${outputVat.toStringAsFixed(0)} ر.س', Colors.black87),
            _kv('ضريبة المدخلات Input VAT', '-${inputVat.toStringAsFixed(0)} ر.س', Colors.black87),
            _kv('التسويات', '${adjustments.toStringAsFixed(0)} ر.س', adjustments >= 0 ? Colors.green : Colors.red),
            const Divider(),
        ...
        Container(
          padding: const EdgeInsets.all(12),
          decoration: BoxDecoration(color: Colors.orange.withOpacity(0.06), borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.orange.withOpacity(0.3))),
          child: const Row(children: [
        ...
        CheckboxListTile(
          ...
          title: const Text('أقرّ بصحة البيانات وأوافق على الإرسال لـ ZATCA', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700)),
          activeColor: _gold,
          dense: true,
        ),
      ]),
      ...
      child: Column(children: [
        ...
        ...lines.map((l) => Container(
              ...
              child: Row(children: [
                ...
                SizedBox(width: 130, child: Text(l.$4.toStringAsFixed(0), style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: _gold), textAlign: TextAlign.end)),
              ]),
            )),
      ]),
    );
  ...
  Widget _kv(String label, String value, Color color, {bool big = false}) {
    return Padding(
      ...
      child: Row(children: [
        Expanded(child: Text(label, style: TextStyle(fontSize: big ? 14 : 12, fontWeight: big ? FontWeight.w800 : FontWeight.w500, color: Colors.black87))),
