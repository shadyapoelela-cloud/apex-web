...
class Supplier360V52Screen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ObjectPageTemplate(
      ...
      smartButtons: const [
        ...
        SmartButton(icon: Icons.shopping_cart, labelAr: 'أمر شراء', count: 124, color: Colors.teal),
        SmartButton(icon: Icons.verified, labelAr: 'شهادة', count: 6, color: Colors.green),
      ...
      primaryActions: [
        OutlinedButton.icon(onPressed: () {}, icon: const Icon(Icons.request_quote, size: 16), label: const Text('طلب عرض سعر')),
        ...
        children: [
          Row(
            children: const [
              ...
              SizedBox(width: 10),
              Expanded(child: _Kpi(label: 'مدفوعات مستحقة', value: '184K', unit: 'ر.س', color: Colors.orange, icon: Icons.payments)),
              SizedBox(width: 10),
              Expanded(child: _Kpi(label: 'متوسط الالتزام', value: '94%', unit: 'OTD', color: Colors.green, icon: Icons.schedule)),
              SizedBox(width: 10),
          ...
          _card('أداء التسليم (آخر 12 شهر)', Column(children: [
            _progRow('الالتزام بالمواعيد (OTD)', 100, 94, '%', Colors.green),
            const SizedBox(height: 10),
            _progRow('جودة المنتجات', 100, 92, '%', _gold),
            const SizedBox(height: 10),
            _progRow('دقة الفواتير', 100, 98, '%', Colors.blue),
            const SizedBox(height: 10),
            _progRow('الاستجابة للطلبات', 100, 89, '%', Colors.orange),
          ])),
        ],
      ),
    );
  ...
  Widget _purchases() {
    const pos = [
      ('PO-2026-124', '2026-04-12', 140000.0, 'مُسلَّم', Colors.green),
      ('PO-2026-118', '2026-04-03', 87000.0, 'مُسلَّم', Colors.green),
      ('PO-2026-087', '2026-03-28', 210000.0, 'قيد التسليم', Colors.blue),
      ('PO-2026-062', '2026-03-14', 45000.0, 'مُسلَّم', Colors.green),
      ('PO-2026-041', '2026-02-28', 68000.0, 'مُسلَّم', Colors.green),
    ];
    return ListView.separated(
      ...
      itemBuilder: (ctx, i) => Container(
        ...
        child: Row(children: [
          const Icon(Icons.shopping_cart, color: _gold),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(pos[i].$1, style: const TextStyle(fontFamily: 'monospace', fontSize: 11, color: Colors.black54)),
            Text('تاريخ الأمر: ${pos[i].$2}', style: const TextStyle(fontSize: 12)),
          ])),
          Text('${pos[i].$3.toStringAsFixed(0)} ر.س', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: _navy)),
          const SizedBox(width: 12),
  ...
  Widget _payments() {
    ...
    return ListView.separated(
      ...
      itemBuilder: (ctx, i) => Container(
        ...
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.grey.shade200)),
        child: Row(children: [
          Icon(Icons.payments, color: pays[i].$5 == 'معلّق' ? Colors.orange : Colors.green),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(pays[i].$1, style: const TextStyle(fontFamily: 'monospace', fontSize: 11, color: Colors.black54)),
            Text('${pays[i].$2} · ${pays[i].$4}', style: const TextStyle(fontSize: 12)),
          ])),
          Text('${pays[i].$3.toStringAsFixed(0)} ر.س', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: _navy)),
          const SizedBox(width: 12),
          Chip(label: Text(pays[i].$5, style: const TextStyle(fontSize: 10)), backgroundColor: (pays[i].$5 == 'معلّق' ? Colors.orange : Colors.green).withOpacity(0.12)),
        ]),
      ),
    );
  }
  ...
  Widget _contracts() {
    const contracts = [
      ('عقد إطاري 2026', '2026-01-01 → 2026-12-31', 2800000.0, 'نشط', Colors.green),
      ('عقد صيانة معدات IT', '2025-07-01 → 2026-06-30', 340000.0, 'نشط', Colors.green),
      ('عقد توريد مواد مكتبية', '2025-03-15 → 2026-03-14', 145000.0, 'منتهي', Colors.grey),
    ];
    return ListView.separated(
      padding: const EdgeInsets.all(24),
      ...
      itemBuilder: (ctx, i) => Container(
        ...
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8), border: Border.all(color: contracts[i].$4 == 'نشط' ? Colors.green.withOpacity(0.3) : Colors.grey.shade300)),
        child: Row(children: [
          Icon(Icons.gavel, color: contracts[i].$5),
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(contracts[i].$1, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w800)),
            Text(contracts[i].$2, style: const TextStyle(fontSize: 11, color: Colors.black54)),
          ])),
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            Text('${contracts[i].$3.toStringAsFixed(0)} ر.س', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: _navy)),
  ...
  Widget _performance() {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('أداء المورد عبر 12 شهر', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: _navy)),
          const SizedBox(height: 16),
          Container(
              ...
              children: [
                for (final m in const [('ينا', 0.92), ('فبر', 0.89), ('مارس', 0.96), ('أبريل', 0.94)])
                  Padding(
                    ...
                    child: Row(
                      children: [
                        SizedBox(width: 40, child: Text(m.$1, style: const TextStyle(fontSize: 11, color: Colors.black54))),
                        Expanded(child: ClipRRect(borderRadius: BorderRadius.circular(4), child: LinearProgressIndicator(value: m.$2, minHeight: 16, backgroundColor: Colors.grey.shade100, color: _gold))),
  ...
  Widget _docs() {
    ...
    return ListView.separated(
      ...
      itemBuilder: (ctx, i) {
        ...
        return Container(
          padding: const EdgeInsets.all(12),
          decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8), border: Border.all(color: urgent ? Colors.red.withOpacity(0.4) : Colors.grey.shade200)),
          child: Row(children: [
            Icon(Icons.verified, color: urgent ? Colors.red : Colors.green),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(docs[i].$1, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w800)),
              Text(docs[i].$2, style: const TextStyle(fontSize: 11, color: Colors.black54)),
            ])),
            Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: (urgent ? Colors.red : Colors.green).withOpacity(0.12), borderRadius: BorderRadius.circular(12)), child: Text('${daysLeft > 0 ? daysLeft : "منتهية"} يوم', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: urgent ? Colors.red : Colors.green))),
          ]),
        );
      },
    );
  ...
  Widget _progRow(String label, double total, double used, String unit, Color color) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      ClipRRect(borderRadius: BorderRadius.circular(4), child: LinearProgressIndicator(value: pct, minHeight: 8, backgroundColor: Colors.grey.shade200, color: color)),
    ]);
  }
}
