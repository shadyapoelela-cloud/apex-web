...
class _CostCentersV52ScreenState extends State<CostCentersV52Screen> {
  ...
  Widget build(BuildContext context) {
    ...
    return MultiViewTemplate(
      ...
      filterChips: [
        FilterChipDef(id: 'onBudget', labelAr: 'ضمن الميزانية', color: Colors.green, count: _cnt(_S.onBudget), active: _filter == 'onBudget'),
        FilterChipDef(id: 'underBudget', labelAr: 'توفير', color: _gold, count: _cnt(_S.underBudget), active: _filter == 'underBudget'),
        FilterChipDef(id: 'overBudget', labelAr: 'تجاوز', color: Colors.red, count: _cnt(_S.overBudget), active: _filter == 'overBudget'),
      ],
        ...
        return Card(
          ...
          child: Padding(
            ...
            child: Row(children: [
              ...
              Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                Text('الفعلي', style: TextStyle(fontSize: 10, color: Colors.grey.shade600)),
              ...
              SizedBox(width: 110, child: Column(children: [
                Row(children: [Text('الاستغلال', style: TextStyle(fontSize: 10, color: Colors.grey.shade600)), const Spacer(), Text('${util.toStringAsFixed(0)}%', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: c.status.color))]),
                const SizedBox(height: 2),
                ClipRRect(borderRadius: BorderRadius.circular(3), child: LinearProgressIndicator(value: (util / 100).clamp(0.0, 1.0), minHeight: 6, backgroundColor: Colors.grey.shade200, color: c.status.color)),
              ])),
              const SizedBox(width: 16),
          ...
          Expanded(child: ListView(children: _centers.map((c) {
            ...
            return Container(
              ...
              child: Row(children: [
                ...
                Expanded(child: Text('${variance >= 0 ? '+' : ''}${(variance / 1000).toStringAsFixed(0)}K', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: variance >= 0 ? Colors.green : Colors.red), textAlign: TextAlign.end)),
  ...
  Widget _chart() {
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('استغلال الميزانية — تنازلياً', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: _navy)),
        const SizedBox(height: 4),
        Text('يظهر الأعلى تجاوزاً في الأعلى', style: TextStyle(fontSize: 11, color: Colors.grey.shade600)),
          ...
          return Padding(padding: const EdgeInsets.only(bottom: 12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              Expanded(child: Stack(children: [
                Container(height: 24, decoration: BoxDecoration(color: Colors.grey.shade100, borderRadius: BorderRadius.circular(3))),
                ...
                if (util > 1.0) Positioned(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(color: Colors.red.withOpacity(0.8), borderRadius: const BorderRadius.only(topRight: Radius.circular(3), bottomRight: Radius.circular(3))),
...
extension _SX on _S {
  ...
  Color get color => switch (this) {
        _S.onBudget => Colors.green,
        _S.underBudget => const Color(0xFFD4AF37),
