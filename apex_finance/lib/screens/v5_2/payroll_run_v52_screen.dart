...
class PayrollRunV52Screen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ObjectPageTemplate(
      ...
      smartButtons: const [
        ...
        SmartButton(icon: Icons.event_available, labelAr: 'إجازة', count: 28, color: Colors.blue),
        SmartButton(icon: Icons.emoji_events, labelAr: 'مكافأة', count: 12, color: _gold),
        SmartButton(icon: Icons.money_off, labelAr: 'خصم', count: 6, color: Colors.red),
        SmartButton(icon: Icons.warning, labelAr: 'استثناء', count: 3, color: Colors.orange),
      ],
      primaryActions: [
        OutlinedButton.icon(onPressed: () {}, icon: const Icon(Icons.visibility, size: 16), label: const Text('معاينة قسائم')),
        const SizedBox(width: 8),
        FilledButton.icon(
          onPressed: () {},
          style: FilledButton.styleFrom(backgroundColor: _gold),
          icon: const Icon(Icons.send, size: 16),
          label: const Text('اعتماد وإرسال WPS'),
        ),
      ],
      tabs: [
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Expanded(child: _bigStat('صافي الصرف', '1,704,200', 'ر.س', Colors.green, Icons.account_balance_wallet)),
          const SizedBox(width: 10),
          Expanded(child: _bigStat('GOSI', '138,300', 'ر.س', Colors.blue, Icons.shield)),
        ]),
        const SizedBox(height: 16),
        _card('توزيع الرواتب حسب القسم', Column(children: [
          _depRow('الإدارة العامة', 12, 340000, _gold),
          _depRow('المبيعات', 38, 520000, Colors.blue),
          _depRow('العمليات', 42, 480000, Colors.green),
          _depRow('التقنية', 28, 380000, _navy),
          _depRow('الموارد البشرية', 12, 95000, Colors.purple),
          _depRow('المحاسبة', 10, 100000, Colors.orange),
        ])),
        const SizedBox(height: 16),
        _card('بنود الراتب الإجمالية', Column(children: [
          _compRow('الأجور الأساسية', 1420000, '77%', _gold),
          _compRow('بدلات (سكن + نقل)', 320000, '17%', Colors.blue),
          _compRow('مكافآت وعمولات', 85000, '5%', Colors.green),
          _compRow('ساعات إضافية', 17500, '1%', Colors.orange),
        ])),
      ]),
    );
  }
  ...
  Widget _employees() {
    const emps = [
      ...
      ('EMP-003', 'خالد إبراهيم', 'مهندس أول', 15200, 2000, 900, Colors.blue),
      ('EMP-004', 'ليلى أحمد', 'محلل مالي', 12800, 1700, 750, Colors.blue),
      ('EMP-005', 'عمر حسن', 'مندوب مبيعات', 8400, 1100, 500, Colors.green),
      ('EMP-006', 'نورة الدوسري', 'محاسب', 9800, 1300, 580, Colors.green),
      ('EMP-007', 'يوسف عمر', 'فني صيانة', 6200, 800, 370, Colors.orange),
      ('EMP-008', 'دينا حسام', 'موظفة خدمة', 5400, 700, 320, Colors.orange),
    ];
        ...
        return Container(
          ...
          child: Row(children: [
            ...
            Expanded(flex: 2, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(emps[i].$1, style: const TextStyle(fontFamily: 'monospace', fontSize: 10, color: Colors.black54)),
              Text(emps[i].$2, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700)),
              Text(emps[i].$3, style: const TextStyle(fontSize: 11, color: Colors.black54)),
            ])),
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
              Text('صافي الراتب', style: TextStyle(fontSize: 10, color: Colors.grey.shade600)),
              Text('${net.toStringAsFixed(0)} ر.س', style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: _gold)),
            ]),
          ]),
        );
      },
  ...
  Widget _psCol(String label, int value, {bool neg = false}) => Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
        ...
        Text('${neg ? '-' : ''}${value.toStringAsFixed(0)}', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: neg ? Colors.red : Colors.black87)),
      ]);
  ...
  Widget _components() {
    ...
    return ListView.separated(
      ...
      itemBuilder: (_, i) => Container(
        ...
        child: Row(children: [
          Icon(comps[i].$3 ? Icons.check_circle : Icons.radio_button_unchecked, color: comps[i].$3 ? Colors.green : Colors.grey, size: 18),
          const SizedBox(width: 10),
          Expanded(flex: 2, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(comps[i].$1, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700)),
            Text(comps[i].$4, style: const TextStyle(fontSize: 10, color: Colors.black54)),
          ])),
          Text('${comps[i].$2.toStringAsFixed(0)} ر.س', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: _navy)),
        ]),
      ),
    );
  }
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _card('📋 تقرير GOSI', Column(children: [
          ...
          _kvRow('حصة الموظف (9.75%)', '136,013 ر.س', Colors.red),
          _kvRow('حصة الشركة (11.75%)', '163,913 ر.س', Colors.orange),
          _kvRow('الإجمالي المستحق', '299,926 ر.س', _gold, bold: true),
          const Divider(height: 20),
          Row(children: [
            Expanded(child: FilledButton.icon(onPressed: () {}, style: FilledButton.styleFrom(backgroundColor: Colors.blue), icon: const Icon(Icons.upload, size: 16), label: const Text('رفع إلى GOSI'))),
            const SizedBox(width: 10),
            Expanded(child: OutlinedButton.icon(onPressed: () {}, icon: const Icon(Icons.download, size: 16), label: const Text('تحميل ملف SIMPLE'))),
          ]),
        ])),
        const SizedBox(height: 16),
        _card('💳 تقرير WPS (نظام حماية الأجور)', Column(children: [
          ...
          _kvRow('SIF File', 'جاهز للإرسال', Colors.green),
          const Divider(height: 20),
          Row(children: [
            Expanded(child: FilledButton.icon(onPressed: () {}, style: FilledButton.styleFrom(backgroundColor: Colors.green), icon: const Icon(Icons.send, size: 16), label: const Text('إرسال SIF إلى البنك'))),
            const SizedBox(width: 10),
            Expanded(child: OutlinedButton.icon(onPressed: () {}, icon: const Icon(Icons.preview, size: 16), label: const Text('معاينة SIF'))),
          ]),
        ])),
      ]),
    );
  ...
  Widget _exceptions() {
    const exc = [
      ('عمر حسن', 'انقطاع 3 أيام غير مبرّر', 'خصم 1,200 ر.س', Colors.orange),
      ('يوسف عمر', 'ساعات إضافية فوق الحد المسموح', 'يحتاج موافقة مدير', Colors.red),
      ('نورة الدوسري', 'تغيير في الراتب (ترقية)', 'زيادة 800 ر.س - موثّقة', Colors.green),
    ];
    return ListView.separated(
      padding: const EdgeInsets.all(24),
  ...
  Widget _kvRow(String k, String v, Color? color, {bool bold = false}) => Padding(padding: const EdgeInsets.symmetric(vertical: 4), child: Row(children: [
        Expanded(child: Text(k, style: const TextStyle(fontSize: 12, color: Colors.black87))),
        Text(v, style: TextStyle(fontSize: bold ? 15 : 13, fontWeight: FontWeight.w800, color: color ?? Colors.black87)),
      ]));
  ...
  Widget _depRow(String dept, int headcount, double total, Color color) => Padding(padding: const EdgeInsets.symmetric(vertical: 6), child: Row(children: [
        ...
        Text('$headcount موظف', style: const TextStyle(fontSize: 11, color: Colors.black54)),
        const SizedBox(width: 16),
