...
class _TbBindingScreenState extends State<TbBindingScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(backgroundColor: _surface,
        title: Text('ربط ميزان المراجعة', style: TextStyle(fontFamily:'Tajawal', color:AC.tp, fontSize:15, fontWeight:FontWeight.w700)),
  ...
  Widget _buildPrompt() => Center(child: Padding(padding: EdgeInsets.all(32), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
    Container(width:72,height:72, decoration: BoxDecoration(color:_gold.withValues(alpha: 0.08), shape:BoxShape.circle, border:Border.all(color:_gold.withValues(alpha: 0.3))), child: Icon(Icons.link_rounded, color:AC.gold, size:34)),
    SizedBox(height:20),
  ...
  Widget _buildResults() {
    ...
    return Column(children: [
      Container(padding: EdgeInsets.all(14), decoration: BoxDecoration(color:AC.navy2, border:Border(bottom:BorderSide(color:AC.bdr))),
        child: Column(children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Text('إجمالي: $total حساب', style: TextStyle(fontSize:12, color:AC.ts, fontFamily:'Tajawal')),
          ]),
          SizedBox(height:8),
          ...
          Row(children: [
            ...
            _sChip('غير مطابق','$unmatched',_danger), SizedBox(width:8),
            _sChip('مراجعة','$review',_warning), SizedBox(width:8),
            _sChip('متوسط الثقة','${(avgConf*100).toStringAsFixed(0)}%',_cyan),
          ]),
        ])),
      Container(height:40, padding:EdgeInsets.symmetric(horizontal:12), decoration:BoxDecoration(color:AC.navy2,border:Border(bottom:BorderSide(color:AC.bdr))),
        child:Row(children:[
          for(final f in [('all','الكل'),('matched','مطابق'),('unmatched','غير مطابق'),('review','مراجعة')]) Padding(padding:EdgeInsets.only(left:8),
            child:GestureDetector(onTap:(){setState(()=>_filter=f.$1);_loadResults(reset:true);},
              child:AnimatedContainer(duration:Duration(milliseconds:150),padding:EdgeInsets.symmetric(horizontal:10,vertical:4),
      ...
      Expanded(child: _loadingResults && _results.isEmpty ? Center(child:CircularProgressIndicator(color:AC.gold))
        : _results.isEmpty ? Center(child:Text('لا توجد نتائج',style:TextStyle(color:AC.ts,fontFamily:'Tajawal')))
        : ListView.builder(padding:EdgeInsets.all(12), itemCount:_results.length+(_results.length<_total?1:0),
            itemBuilder:(_,i){
              if(i==_results.length) return Padding(padding:EdgeInsets.symmetric(vertical:16),child:Center(child:TextButton(onPressed:(){setState(()=>_page++);_loadResults();},child:Text('تحميل المزيد',style:TextStyle(color:AC.gold,fontFamily:'Tajawal')))));
              final r=_results[i];
              ...
              return Container(margin:EdgeInsets.only(bottom:8),padding:const EdgeInsets.all(12),
                ...
                child:Column(children:[
                  ...
                  Row(children:[
                    ...
                    SizedBox(width:4),Text('صافي',style:TextStyle(fontSize:10,color:AC.ts,fontFamily:'Tajawal')),
                    Spacer(),
                    SizedBox(width:60,child:Column(crossAxisAlignment:CrossAxisAlignment.end,children:[Text('${(conf*100).toInt()}%',style:TextStyle(fontSize:10,color:color,fontFamily:'Tajawal')),SizedBox(height:2),ClipRRect(borderRadius:BorderRadius.circular(2),child:LinearProgressIndicator(value:conf.clamp(0.0,1.0),minHeight:3,backgroundColor:_border,valueColor:AlwaysStoppedAnimation(color)))])),
                  ]),
                  if(matched&&r['coa_class']!=null)...[SizedBox(height:5),Align(alignment:Alignment.centerRight,child:Container(padding:EdgeInsets.symmetric(horizontal:7,vertical:2),decoration:BoxDecoration(color:_gold.withValues(alpha: 0.06),borderRadius:BorderRadius.circular(4),border:Border.all(color:_gold.withValues(alpha: 0.15))),child:Text('${r["coa_class"]} — ${r["coa_section"]??""}',style:TextStyle(fontSize:10,color:AC.gold,fontFamily:'Tajawal'))))],
                ]));
            })),
    ]);
  }
  ...
  Widget _sChip(String label, String value, Color color) => Expanded(child: Container(padding: EdgeInsets.symmetric(vertical:7), decoration: BoxDecoration(color:color.withValues(alpha: 0.06), borderRadius:BorderRadius.circular(8), border:Border.all(color:color.withValues(alpha: 0.2))), child: Column(children: [Text(value,style:TextStyle(fontSize:14,fontWeight:FontWeight.w800,color:color,fontFamily:'Tajawal')),SizedBox(height:2),Text(label,textAlign:TextAlign.center,style:TextStyle(fontSize:9,color:AC.ts,fontFamily:'Tajawal'))])));
}
