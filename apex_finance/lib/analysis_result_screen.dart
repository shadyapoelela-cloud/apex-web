...
class _AnalysisResultScreenState extends State<AnalysisResultScreen> with TickerProviderStateMixin {
  ...
  static Color get _bg=>AC.navy;static Color get _surface=>AC.navy2;static Color get _card=>AC.navy3;static Color get _gold=>AC.gold;static Color _cyan=Color(0xFF00C2E0);static const Color _success=Color(0xFF2ECC8A);static const Color _danger=Color(0xFFE05050);static const Color _warning=Color(0xFFE8A838);static Color get _border=>AC.bdr;static Color get _textPri=>AC.tp;static Color get _textSec=>AC.ts;
  ...
  void _showDetailPanel(BuildContext context, Map ratio){
    showModalBottomSheet(context:context,backgroundColor:Colors.transparent,isScrollControlled:true,builder:(_)=>DraggableScrollableSheet(initialChildSize:0.72,minChildSize:0.4,maxChildSize:0.95,builder:(_,ctrl)=>Container(decoration:BoxDecoration(color:Color(0xFF0D1829),borderRadius:BorderRadius.vertical(top:Radius.circular(24))),child:Column(children:[
      ...
      Expanded(child:ListView(controller:ctrl,padding:const EdgeInsets.all(20),children:[
        ...
        Row(children:[
          Expanded(child:_infoCard('مستوى الثقة','${((ratio['confidence']??ratio['score']??0)as num).toStringAsFixed(0)}%',Color(0xFF2ECC8A))),
          SizedBox(width:10),
          Expanded(child:_infoCard('مستوى الخطر',(ratio['risk_level']??ratio['status'])=='low'?'منخفض':(ratio['risk_level']??ratio['status'])=='medium'?'متوسط':'مرتفع',Color(0xFFE8A838))),
        ]),SizedBox(height:14),
        if((ratio['requires_human_review'])==true)...[Container(padding:EdgeInsets.all(12),decoration:BoxDecoration(color:_danger.withValues(alpha: 0.08),borderRadius:BorderRadius.circular(10),border:Border.all(color:_danger.withValues(alpha: 0.3))),child:Row(children:[Icon(Icons.person_search_rounded,color:Color(0xFFE05050),size:18),SizedBox(width:8),Expanded(child:Text('هذه النتيجة تتطلب مراجعة بشرية قبل الاعتماد',textDirection:TextDirection.rtl,style:TextStyle(fontSize:12,color:Color(0xFFE05050),fontFamily:'Tajawal')))])),const SizedBox(height:14)],
        if((ratio['interpretation']??'').isNotEmpty)...[_sectionHeader('التفسير'),SizedBox(height:8),Container(padding:EdgeInsets.all(14),decoration:BoxDecoration(color:Color(0xFF111E32),borderRadius:BorderRadius.circular(10),border:Border.all(color:_border)),child:Text(ratio['interpretation'],textDirection:TextDirection.rtl,style:TextStyle(fontSize:13,color:AC.tp,fontFamily:'Tajawal',height:1.6))),SizedBox(height:14)],
